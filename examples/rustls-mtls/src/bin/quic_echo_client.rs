...
use rustls_mtls::{initialize_logger, into_certificate, into_private_key};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let client = Client::builder()
        ...
        .with_tls(s2n_quic_rustls::Client::builder()
            .with_application_protocols(vec!["h3"].into_iter())?
            .with_certificate(Path::new(CACERT_PEM))?
            .with_client_identity(Path::new(MY_CERT_PEM), Path::new(MY_KEY_PEM))?
            .build()?)?
        .with_io("0.0.0.0:0")?
        .start()?;
