...
use crate::{buffer::Buffer, dissect, field, field::Registration, wireshark::Item, wireshark_sys};
...
pub fn copy_to_rust(tvb: *mut wireshark_sys::tvbuff_t) -> Vec<u8> {
    let len = unsafe { wireshark_sys::tvb_reported_length(tvb) as usize };
    let mut buffer: Vec<u8> = vec![0; len];
    unsafe {
        wireshark_sys::tvb_memcpy(tvb, buffer.as_mut_ptr() as *mut std::ffi::c_void, 0, len);
    }
    buffer
}
