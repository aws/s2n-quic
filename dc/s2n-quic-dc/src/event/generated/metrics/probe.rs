    ...
    impl Recorder {
        pub(crate) fn new(info: &'static Info) -> Self {
            match info.id {
                ...
                33usize => Self(acceptor_udp_started),
                34usize => Self(acceptor_udp_datagram_received),
                36usize => Self(acceptor_udp_packet_received),
                42usize => Self(acceptor_udp_packet_dropped),
                44usize => Self(acceptor_udp_stream_enqueued),
                45usize => Self(acceptor_udp_io_error),
                46usize => Self(acceptor_stream_pruned),
                49usize => Self(acceptor_stream_dequeued),
                51usize => Self(stream_write_flushed),
                55usize => Self(stream_write_flushed__committed__total),
                60usize => Self(stream_write_fin_flushed),
                64usize => Self(stream_write_fin_flushed__committed__total),
                69usize => Self(stream_write_blocked),
                75usize => Self(stream_write_errored),
                80usize => Self(stream_write_key_updated),
                81usize => Self(stream_write_allocated),
                85usize => Self(stream_write_shutdown),
                89usize => Self(stream_write_socket_flushed),
                92usize => Self(stream_write_socket_flushed__committed__total),
                95usize => Self(stream_write_socket_blocked),
                98usize => Self(stream_write_socket_errored),
                100usize => Self(stream_read_flushed),
                104usize => Self(stream_read_flushed__committed__total),
                109usize => Self(stream_read_fin_flushed),
                115usize => Self(stream_read_blocked),
                120usize => Self(stream_read_errored),
                125usize => Self(stream_read_key_updated),
                126usize => Self(stream_read_shutdown),
                129usize => Self(stream_read_socket_flushed),
                132usize => Self(stream_read_socket_flushed__committed__total),
                135usize => Self(stream_read_socket_blocked),
                138usize => Self(stream_read_socket_errored),
                140usize => Self(stream_decrypt_packet),
                144usize => Self(stream_tcp_connect),
                147usize => Self(stream_connect),
                151usize => Self(stream_connect_error),
                153usize => Self(stream_packet_transmitted),
                155usize => Self(stream_packet_transmitted__payload_len__total),
                159usize => Self(stream_probe_transmitted),
                161usize => Self(stream_packet_received),
                163usize => Self(stream_packet_received__payload_len__total),
                167usize => Self(stream_packet_lost),
                169usize => Self(stream_packet_lost__payload_len__total),
                174usize => Self(stream_packet_acked),
                176usize => Self(stream_packet_acked__payload_len__total),
                181usize => Self(stream_packet_spuriously_retransmitted),
                183usize => Self(stream_packet_spuriously_retransmitted__payload_len__total),
                187usize => Self(stream_max_data_received),
                188usize => Self(stream_max_data_received__increase__total),
                190usize => Self(stream_control_packet_transmitted),
                193usize => Self(stream_control_packet_received),
                197usize => Self(stream_receiver_errored),
                198usize => Self(stream_sender_errored),
                199usize => Self(connection_closed),
                200usize => Self(endpoint_initialized),
                205usize => Self(path_secret_map_initialized),
                207usize => Self(path_secret_map_uninitialized),
                211usize => Self(path_secret_map_background_handshake_requested),
                213usize => Self(path_secret_map_entry_inserted),
                215usize => Self(path_secret_map_entry_ready),
                217usize => Self(path_secret_map_entry_replaced),
                219usize => Self(path_secret_map_id_entry_evicted),
                222usize => Self(path_secret_map_address_entry_evicted),
                225usize => Self(unknown_path_secret_packet_sent),
                227usize => Self(unknown_path_secret_packet_received),
                229usize => Self(unknown_path_secret_packet_accepted),
                231usize => Self(unknown_path_secret_packet_rejected),
                233usize => Self(unknown_path_secret_packet_dropped),
                235usize => Self(key_accepted),
                238usize => Self(replay_definitely_detected),
                239usize => Self(replay_potentially_detected),
                241usize => Self(replay_detected_packet_sent),
                243usize => Self(replay_detected_packet_received),
                245usize => Self(replay_detected_packet_accepted),
                247usize => Self(replay_detected_packet_rejected),
                249usize => Self(replay_detected_packet_dropped),
                251usize => Self(stale_key_packet_sent),
                253usize => Self(stale_key_packet_received),
                255usize => Self(stale_key_packet_accepted),
                257usize => Self(stale_key_packet_rejected),
                259usize => Self(stale_key_packet_dropped),
                261usize => Self(path_secret_map_address_cache_accessed),
                264usize => Self(path_secret_map_address_cache_accessed_hit),
                267usize => Self(path_secret_map_id_cache_accessed),
                269usize => Self(path_secret_map_id_cache_accessed_hit),
                271usize => Self(path_secret_map_cleaner_cycled),
        ...
        impl Recorder {
            pub(crate) fn new(info: &'static Info) -> Self {
                match info.id {
                    ...
                    38usize => Self(acceptor_udp_packet_received__is_zero_offset),
                    39usize => Self(acceptor_udp_packet_received__is_retransmission),
                    40usize => Self(acceptor_udp_packet_received__is_fin),
                    41usize => Self(acceptor_udp_packet_received__is_fin_known),
                    88usize => Self(stream_write_shutdown__background),
                    128usize => Self(stream_read_shutdown__background),
                    141usize => Self(stream_decrypt_packet__decrypted_in_place),
                    145usize => Self(stream_tcp_connect__error),
                    148usize => Self(stream_connect__error),
                    158usize => Self(stream_packet_transmitted__retransmission),
                    166usize => Self(stream_packet_received__retransmission),
                    173usize => Self(stream_packet_lost__retransmission),
                    180usize => Self(stream_packet_acked__retransmission),
                    186usize => Self(stream_packet_spuriously_retransmitted__retransmission),
                    196usize => Self(stream_control_packet_received__authenticated),
                    203usize => Self(endpoint_initialized__tcp),
                    204usize => Self(endpoint_initialized__udp),
                    263usize => Self(path_secret_map_address_cache_accessed__hit),
                    268usize => Self(path_secret_map_id_cache_accessed__hit),
                    _ => unreachable!("invalid info: {info:?}"),
                }
            }
        }
        ...
        impl Recorder {
            pub(crate) fn new(info: &'static Info, _variant: &'static info::Variant) -> Self {
                match info.id {
                    ...
                    43usize => Self(acceptor_udp_packet_dropped__reason),
                    48usize => Self(acceptor_stream_pruned__reason),
                    149usize => Self(stream_connect__tcp),
                    150usize => Self(stream_connect__handshake),
                    152usize => Self(stream_connect_error__reason),
                    201usize => Self(endpoint_initialized__acceptor__protocol),
                    202usize => Self(endpoint_initialized__handshake__protocol),
                    212usize => {
                        Self(path_secret_map_background_handshake_requested__peer_address__protocol)
                    }
                    214usize => Self(path_secret_map_entry_inserted__peer_address__protocol),
                    216usize => Self(path_secret_map_entry_ready__peer_address__protocol),
                    218usize => Self(path_secret_map_entry_replaced__peer_address__protocol),
                    220usize => Self(path_secret_map_id_entry_evicted__peer_address__protocol),
                    223usize => Self(path_secret_map_address_entry_evicted__peer_address__protocol),
                    226usize => Self(unknown_path_secret_packet_sent__peer_address__protocol),
                    228usize => Self(unknown_path_secret_packet_received__peer_address__protocol),
                    230usize => Self(unknown_path_secret_packet_accepted__peer_address__protocol),
                    232usize => Self(unknown_path_secret_packet_rejected__peer_address__protocol),
                    234usize => Self(unknown_path_secret_packet_dropped__peer_address__protocol),
                    242usize => Self(replay_detected_packet_sent__peer_address__protocol),
                    244usize => Self(replay_detected_packet_received__peer_address__protocol),
                    246usize => Self(replay_detected_packet_accepted__peer_address__protocol),
                    248usize => Self(replay_detected_packet_rejected__peer_address__protocol),
                    250usize => Self(replay_detected_packet_dropped__peer_address__protocol),
                    252usize => Self(stale_key_packet_sent__peer_address__protocol),
                    254usize => Self(stale_key_packet_received__peer_address__protocol),
                    256usize => Self(stale_key_packet_accepted__peer_address__protocol),
                    258usize => Self(stale_key_packet_rejected__peer_address__protocol),
                    260usize => Self(stale_key_packet_dropped__peer_address__protocol),
                    262usize => {
...
mod measure {
    ...
    impl Recorder {
        pub(crate) fn new(info: &'static Info) -> Self {
            match info.id {
                ...
                31usize => Self(acceptor_tcp_socket_sent__blocked_count),
                32usize => Self(acceptor_tcp_socket_sent__len),
                35usize => Self(acceptor_udp_datagram_received__len),
                37usize => Self(acceptor_udp_packet_received__payload_len),
                53usize => Self(stream_write_flushed__conn),
                54usize => Self(stream_write_flushed__provided),
                56usize => Self(stream_write_flushed__committed),
                57usize => Self(stream_write_flushed__committed__conn),
                58usize => Self(stream_write_flushed__processing_duration),
                59usize => Self(stream_write_flushed__processing_duration__conn),
                62usize => Self(stream_write_fin_flushed__conn),
                63usize => Self(stream_write_fin_flushed__provided),
                65usize => Self(stream_write_fin_flushed__committed),
                66usize => Self(stream_write_fin_flushed__committed__conn),
                67usize => Self(stream_write_fin_flushed__processing_duration),
                68usize => Self(stream_write_fin_flushed__processing_duration__conn),
                71usize => Self(stream_write_blocked__conn),
                72usize => Self(stream_write_blocked__provided),
                73usize => Self(stream_write_blocked__processing_duration),
                74usize => Self(stream_write_blocked__processing_duration__conn),
                77usize => Self(stream_write_errored__provided),
                78usize => Self(stream_write_errored__processing_duration),
                79usize => Self(stream_write_errored__processing_duration__conn),
                82usize => Self(stream_write_allocated__conn),
                83usize => Self(stream_write_allocated__allocated_len),
                84usize => Self(stream_write_allocated__allocated_len__conn),
                87usize => Self(stream_write_shutdown__buffer_len),
                90usize => Self(stream_write_socket_flushed__conn),
                91usize => Self(stream_write_socket_flushed__provided),
                93usize => Self(stream_write_socket_flushed__committed),
                94usize => Self(stream_write_socket_flushed__committed__conn),
                96usize => Self(stream_write_socket_blocked__conn),
                97usize => Self(stream_write_socket_blocked__provided),
                99usize => Self(stream_write_socket_errored__provided),
                102usize => Self(stream_read_flushed__conn),
                103usize => Self(stream_read_flushed__capacity),
                105usize => Self(stream_read_flushed__committed),
                106usize => Self(stream_read_flushed__committed__conn),
                107usize => Self(stream_read_flushed__processing_duration),
                108usize => Self(stream_read_flushed__processing_duration__conn),
                111usize => Self(stream_read_fin_flushed__conn),
                112usize => Self(stream_read_fin_flushed__capacity),
                113usize => Self(stream_read_fin_flushed__processing_duration),
                114usize => Self(stream_read_fin_flushed__processing_duration__conn),
                117usize => Self(stream_read_blocked__capacity),
                118usize => Self(stream_read_blocked__processing_duration),
                119usize => Self(stream_read_blocked__processing_duration__conn),
                122usize => Self(stream_read_errored__capacity),
                123usize => Self(stream_read_errored__processing_duration),
                124usize => Self(stream_read_errored__processing_duration__conn),
                130usize => Self(stream_read_socket_flushed__conn),
                131usize => Self(stream_read_socket_flushed__capacity),
                133usize => Self(stream_read_socket_flushed__committed),
                134usize => Self(stream_read_socket_flushed__committed__conn),
                136usize => Self(stream_read_socket_blocked__conn),
                137usize => Self(stream_read_socket_blocked__capacity),
                139usize => Self(stream_read_socket_errored__capacity),
                142usize => Self(stream_decrypt_packet__forced_copy),
                143usize => Self(stream_decrypt_packet__required_application_buffer),
                154usize => Self(stream_packet_transmitted__packet_len),
                156usize => Self(stream_packet_transmitted__payload_len),
                157usize => Self(stream_packet_transmitted__payload_len__conn),
                160usize => Self(stream_probe_transmitted__packet_len),
                162usize => Self(stream_packet_received__packet_len),
                164usize => Self(stream_packet_received__payload_len),
                165usize => Self(stream_packet_received__payload_len__conn),
                168usize => Self(stream_packet_lost__packet_len),
                170usize => Self(stream_packet_lost__payload_len),
                171usize => Self(stream_packet_lost__payload_len__conn),
                172usize => Self(stream_packet_lost__lifetime),
                175usize => Self(stream_packet_acked__packet_len),
                177usize => Self(stream_packet_acked__payload_len),
                178usize => Self(stream_packet_acked__payload_len__conn),
                179usize => Self(stream_packet_acked__lifetime),
                182usize => Self(stream_packet_spuriously_retransmitted__packet_len),
                184usize => Self(stream_packet_spuriously_retransmitted__payload_len),
                185usize => Self(stream_packet_spuriously_retransmitted__payload_len__conn),
                189usize => Self(stream_max_data_received__increase),
                191usize => Self(stream_control_packet_transmitted__packet_len),
                192usize => Self(stream_control_packet_transmitted__control_data_len),
                194usize => Self(stream_control_packet_received__packet_len),
                195usize => Self(stream_control_packet_received__control_data_len),
                206usize => Self(path_secret_map_initialized__capacity),
                208usize => Self(path_secret_map_uninitialized__capacity),
                209usize => Self(path_secret_map_uninitialized__entries),
                210usize => Self(path_secret_map_uninitialized__lifetime),
                221usize => Self(path_secret_map_id_entry_evicted__age),
                224usize => Self(path_secret_map_address_entry_evicted__age),
                236usize => Self(key_accepted__gap),
                237usize => Self(key_accepted__forward_shift),
                240usize => Self(replay_potentially_detected__gap),
                266usize => Self(path_secret_map_address_cache_accessed_hit__age),
                270usize => Self(path_secret_map_id_cache_accessed_hit__age),
                272usize => Self(path_secret_map_cleaner_cycled__entries__id),
                273usize => Self(path_secret_map_cleaner_cycled__entries__id__retired),
                274usize => Self(path_secret_map_cleaner_cycled__entries__id__active),
                275usize => Self(path_secret_map_cleaner_cycled__entries__id__active__utilization),
                276usize => Self(path_secret_map_cleaner_cycled__entries__id__utilization),
                277usize => Self(path_secret_map_cleaner_cycled__entries__id__utilization__initial),
                278usize => Self(path_secret_map_cleaner_cycled__entries__address),
                279usize => Self(path_secret_map_cleaner_cycled__entries__address__active),
                280usize => {
                    Self(path_secret_map_cleaner_cycled__entries__address__active__utilization)
                }
                281usize => Self(path_secret_map_cleaner_cycled__entries__address__retired),
                282usize => Self(path_secret_map_cleaner_cycled__entries__address__utilization),
                283usize => {
                    Self(path_secret_map_cleaner_cycled__entries__address__utilization__initial)
                }
                284usize => Self(path_secret_map_cleaner_cycled__handshake_requests),
                285usize => Self(path_secret_map_cleaner_cycled__handshake_requests__retired),
                286usize => Self(path_secret_map_cleaner_cycled__handshake_lock_duration),
                287usize => Self(path_secret_map_cleaner_cycled__total_duration),
                _ => unreachable!("invalid info: {info:?}"),
            }
        }
    }
    ...
    define!(
        extern "probe" {
            ...
            # [link_name = s2n_quic_dc__event__measure__acceptor_tcp_socket_sent__blocked_count]
            fn acceptor_tcp_socket_sent__blocked_count(value: u64);
    ...
    impl Recorder {
        pub(crate) fn new(info: &'static Info) -> Self {
            match info.id {
                ...
                47usize => Self(acceptor_stream_pruned__sojourn_time),
                50usize => Self(acceptor_stream_dequeued__sojourn_time),
                52usize => Self(stream_write_flushed__latency),
                61usize => Self(stream_write_fin_flushed__latency),
                70usize => Self(stream_write_blocked__latency),
                76usize => Self(stream_write_errored__latency),
                86usize => Self(stream_write_shutdown__latency),
                101usize => Self(stream_read_flushed__latency),
                110usize => Self(stream_read_fin_flushed__latency),
                116usize => Self(stream_read_blocked__latency),
                121usize => Self(stream_read_errored__latency),
                127usize => Self(stream_read_shutdown__latency),
                146usize => Self(stream_tcp_connect__tcp_latency),
                _ => unreachable!("invalid info: {info:?}"),
            }
        }
    }
