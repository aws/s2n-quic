...
use s2n_quic_core::{
    ...
    ready,
    stream::state::Receiver as State,
    time::{
...
impl Receiver {
    ...
    pub fn stop_sending(&mut self, error: s2n_quic_core::application::Error) {
        ...
        ensure!(matches!(self.state, State::Recv | State::SizeKnown));
    ...
    #[inline]
    fn on_stream_packet_in_place<D, Clk>(
        &mut self,
        crypto: &D,
        packet: &mut stream::decoder::Packet,
        ecn: ExplicitCongestionNotification,
    ...
    #[inline]
    fn on_stream_packet_copy<D, Clk>(
        &mut self,
        crypto: &D,
        packet: &mut stream::decoder::Packet,
        ecn: ExplicitCongestionNotification,
        ...
        Clk: Clock + ?Sized,
    {
        ...
        if matches!(self.state, State::Recv | State::SizeKnown)
    ...
    pub fn check_error(&self) -> Result<(), Error> {
        // if we already received/read all of the data then filter out errors
            ...
            !matches!(self.state, State::DataRead | State::DataRecvd),
            Ok(())
        );
...
impl timer::Provider for Receiver {
    #[inline]
