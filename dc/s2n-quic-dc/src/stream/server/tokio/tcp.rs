    ...
    Sub: event::Subscriber + Clone,
{
    ...
    pub async fn run(mut self) {
        ...
        let mut workers = {
            let workers = (0..self.backlog)
                .map(|idx| {
                    worker::Worker::new(self.env.clock().get_time(), self.waker_set.waker(idx))
                })
                .collect();
        ...
        poll_fn(move |cx| {
            ...
            let res = workers.poll(self.waker_set.drain(), &mut context, &publisher, &now);
