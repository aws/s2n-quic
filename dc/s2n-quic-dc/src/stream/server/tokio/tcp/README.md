...
By default we bind min(# of vCPUs, 4) listening sockets to the configured
listening address. On Linux, each socket is configured with `TCP_DEFER_ACCEPT`
in order to reduce CPU churn when accepting sockets. Each listening socket has
a Tokio task which:

1. Calls accept() up to 2x the per-worker stream queue capacity from the
   listening socket.
   - Enqueues into stream queue (capacity is the configured socket backlog
     (default `SOMAXCONN`) divided by min(concurrency, 4)). Retains more
     recent sockets if we overflow.
   - See fresh.rs.
