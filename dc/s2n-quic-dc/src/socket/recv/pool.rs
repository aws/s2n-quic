...
impl Pool {
    ...
    pub fn new(max_packet_size: u16, packet_count: usize) -> Self {
        ...
        let mut memory = Memory::new(max_packet_size, Arc::downgrade(&free), region);
        let memory_ptr = unsafe { NonNull::new_unchecked(memory.as_mut()) };

        for idx in 0..packet_count {
            let offset = packet.size() * idx;
            unsafe {
                let descriptor = ptr.add(offset).cast::<DescriptorInner>();
                let addr = ptr.add(offset + addr_offset).cast::<Addr>();
                let data = ptr.add(offset + packet_offset);
                ...
                descriptor.write(DescriptorInner::new(idx as _, addr, data, memory_ptr));
...
mod tests {
    use super::*;
    use crate::socket::recv::descriptor::Filled;
