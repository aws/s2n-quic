...
impl Drain {
    pub fn new(slots: Arc<[Arc<Slot>]>) -> Self {
        Self {
            slots,
...
/// across drains so each drain polls a roughly equal share.
pub(crate) fn new(num_slots: usize, num_drains: usize) -> (Vec<Sink>, Vec<Drain>) {
    ...
    let sinks: Vec<Sink> = slots.iter().map(|s| Sink { slot: s.clone() }).collect();
