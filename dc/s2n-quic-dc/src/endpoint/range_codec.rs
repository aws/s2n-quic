...
/// # Panics
///
/// Debug-asserts that ranges are well-formed (start <= end, descending, non-adjacent).
#[inline]
pub fn encode<E: Encoder>(
    ...
    for range in ranges {
        ...
        if let Some(prev) = prev_smallest {
            debug_assert!(
                end.as_u64() + 2 <= prev.as_u64(),
                "ranges must be descending and non-adjacent"
            );
...
///
/// Empty payload is treated as a single-value range `largest..=largest`. This is
/// equivalent to a payload containing `first_range = 0` but saves one byte on the
/// wire for the common "one slot freed" case.
#[derive(Clone, Copy)]
...
mod tests {
    ...
    #[test]
    fn encode_partial_respects_budget() {
        let mut set = IntervalSet::new();
        let _ = set.insert(VarInt::from_u8(3)..=VarInt::from_u8(5));
        let _ = set.insert(VarInt::from_u8(8)..=VarInt::from_u8(10));
        let _ = set.insert(VarInt::from_u8(15)..=VarInt::from_u8(20));
        ...
        // Budget of 1 byte: only the first_range (value 5, encodes in 1 byte) fits
        let mut buf = vec![0u8; 256];
        let len = {
            let mut encoder = s2n_codec::EncoderBuffer::new(&mut buf);
            let mut iter = ranges_desc.clone().into_iter().peekable();
            let count = encode_partial(largest, &mut iter, &mut encoder, 1);
            assert_eq!(count, 1);
            // Remaining ranges are still in the iterator
        ...
        let decoded: Vec<_> = decoder.map(|r| r.unwrap()).collect();
        assert_eq!(decoded.len(), 1);
        assert_eq!(*decoded[0].start(), VarInt::from_u8(15));
        assert_eq!(*decoded[0].end(), VarInt::from_u8(20));

        // Unlimited budget: all ranges fit
        let mut buf = vec![0u8; 256];
        let len = {
            let mut encoder = s2n_codec::EncoderBuffer::new(&mut buf);
            let mut iter = ranges_desc.into_iter().peekable();
            let count = encode_partial(largest, &mut iter, &mut encoder, 256);
            assert_eq!(count, 3);
