...
//!
//! Everything about how an entry's own state is laid out on disk lives here. The extract side returns bytes
//! ([`Entry::to_persisted_bytes`]); the restore side takes bytes
//! ([`Map::insert_persisted`](crate::path::secret::map::Map::insert_persisted)). The embedding
//! application frames, checksums, versions its file, and schedules writes around these blobs, but
//! never inspects or constructs their contents.
//!
...
//! than silently altering what `V1` means. An embedding application cannot desync from a layout it
//! never sees.
//!
...
/// to an older generation rather than misread key material.
...
/// Caller-tunable knobs for restoring an entry. `s2n-quic-dc` clamps each to its own security
...
struct PersistedStateV1 {
    ...
    /// no assumption about fleet homogeneity.
    remote_max_data: u64,
}
...
/// the invariants, so it owns this classification; the caller obeys it
...
pub enum RestoreDisposition {
    ...
    /// it, and continue with the next record. The worst case is a smaller map, never a failed
    /// startup.
    SkipEntry,
...
/// panics; nothing here aborts startup. A corrupt file must at worst leave the map empty.
...
pub enum InsertPersistedError {
    ...
    /// is the check that keeps a corrupted sender/receiver counter from silently producing an entry
    /// that reuses key material or accepts replayed packets.
...
impl InsertPersistedError {
    ...
    pub fn disposition(&self) -> RestoreDisposition {
        ...
        match self {
            // Framing intact, one entry unusable: skip and keep loading.
            ...
            // file and fall back to an older generation. This is the rollback escape hatch.
...
/// `crc32fast`, so a value computed here agrees with one an embedding application might compute over the same
...
mod tests {
    ...
    const SENDER_FLOOR: u64 = 1_000_000 * 5 * 24 * 60 * 60;
    const RECEIVER_FLOOR: u64 = 1_000_000 * 5 * 24 * 60 * 60;
    ...
    fn advance_over_floor_is_honoured() {
        // Must exceed SENDER_FLOOR / RECEIVER_FLOOR to test that the caller's advance wins.
        let advance = SENDER_FLOOR * 2;
