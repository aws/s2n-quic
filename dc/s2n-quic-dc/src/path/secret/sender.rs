...
/// counter, and a host crash loses any issuance since the last write.
///
...
/// cannot defeat the key-schedule invariant.
...
pub enum RestoreError {
    ...
    /// for this entry. Practically unreachable at the given floor -- it needs a persisted counter
    /// within range of 2^62 -- but restoring it verbatim would risk key-id reuse, so the entry is
    /// rejected instead.
