...
/// gap. `2^32` subsumes it. Like the sender's floor this is applied under `max(advance, floor)`, so
/// a too-small caller `advance` cannot cause replay acceptance (see the plan's C5).
...
impl State {
    ...
    /// [`MIN_RESTORE_ADVANCE`] (2^32) must cover that unpersisted gap -- not merely the `WINDOW`
    ...
    /// caller cannot get it wrong (see the plan's C5).
