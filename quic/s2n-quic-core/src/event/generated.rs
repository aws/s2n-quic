...
pub mod api {
    ...
    #[non_exhaustive]
    #[doc = " Path challenge abandoned"]
    pub struct PathValidationProbing {
        pub frame: Frame,
    }
    impl Event for PathValidationProbing {
        const NAME: &'static str = "connectivity:path_validation_probing";
    }
    #[derive(Clone, Debug)]
    #[non_exhaustive]
    #[doc = " QUIC version"]
...
pub mod builder {
    ...
    impl<'a> IntoEvent<api::PathChallengeAbandoned<'a>> for PathChallengeAbandoned<'a> {
        ...
        fn into_event(self) -> api::PathChallengeAbandoned<'a> {
            let PathChallengeAbandoned { path } = self;
    ...
    impl IntoEvent<api::PathValidationProbing> for PathValidationProbing {
        #[inline]
        fn into_event(self) -> api::PathValidationProbing {
            let PathValidationProbing { frame } = self;
            api::PathValidationProbing {
                frame: frame.into_event(),
            }
        }
    }
...
mod traits {
    ...
    pub trait Subscriber: 'static + Send {
        ...
        #[doc = "Called when the `PathValidationProbing` event is triggered"]
        #[inline]
        fn on_path_validation_probing(
            &mut self,
            context: &mut Self::ConnectionContext,
            meta: &ConnectionMeta,
            event: &PathValidationProbing,
        ) {
            let _ = context;
            let _ = meta;
            let _ = event;
        ...
        B: Subscriber,
    {
        ...
        fn on_path_validation_probing(
            ...
            (self.0).on_path_validation_probing(&mut context.0, meta, event);
            (self.1).on_path_validation_probing(&mut context.1, meta, event);
        }
        #[inline]
        fn on_version_information(&mut self, meta: &EndpointMeta, event: &VersionInformation) {
    ...
    pub trait ConnectionPublisher {
        ...
        #[doc = "Publishes a `PathValidationProbing` event to the publisher's subscriber"]
        fn on_path_validation_probing(&mut self, event: builder::PathValidationProbing);
    ...
    impl<'a, Sub: Subscriber> ConnectionPublisher for ConnectionPublisherSubscriber<'a, Sub> {
        ...
        fn on_path_challenge_abandoned(&mut self, event: builder::PathChallengeAbandoned) {
            let event = event.into_event();
            self.subscriber
                .on_path_challenge_abandoned(&mut self.context, &self.meta, &event);
            self.subscriber
                .on_connection_event(&mut self.context, &self.meta, &event);
            self.subscriber.on_event(&self.meta, &event);
        }
        ...
        fn on_path_validation_probing(&mut self, event: builder::PathValidationProbing) {
            let event = event.into_event();
            self.subscriber
                .on_path_validation_probing(&mut self.context, &self.meta, &event);
            self.subscriber
                .on_connection_event(&mut self.context, &self.meta, &event);
            self.subscriber.on_event(&self.meta, &event);
        }
...
pub mod testing {
    ...
    pub struct Subscriber {
        ...
        pub path_validation_probing: u32,
    ...
    impl super::Subscriber for Subscriber {
        ...
        fn on_path_challenge_abandoned(
            ...
            _context: &mut Self::ConnectionContext,
            _meta: &api::ConnectionMeta,
            _event: &api::PathChallengeAbandoned,
        ) {
        ...
        fn on_path_validation_probing(
            ...
            _context: &mut Self::ConnectionContext,
            _meta: &api::ConnectionMeta,
            _event: &api::PathValidationProbing,
        ) {
            self.path_validation_probing += 1;
        }
    ...
    impl super::ConnectionPublisher for Publisher {
        ...
        fn on_path_validation_probing(&mut self, _event: builder::PathValidationProbing) {
            self.path_validation_probing += 1;
        }
        fn quic_version(&self) -> u32 {
            1
