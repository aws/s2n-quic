...
mod counter {
    ...
    impl Recorder {
        pub(crate) fn new(info: &'static Info) -> Self {
            match info.id {
                ...
                9usize => Self(packet_received),
                11usize => Self(active_path_updated),
                12usize => Self(path_created),
                13usize => Self(frame_sent),
                16usize => Self(frame_received),
                19usize => Self(connection_close_frame_received),
                21usize => Self(packet_lost),
                23usize => Self(packet_lost__bytes__total),
                26usize => Self(recovery_metrics),
                36usize => Self(congestion),
                38usize => Self(rx_ack_range_dropped),
                39usize => Self(ack_range_received),
                41usize => Self(ack_range_sent),
                43usize => Self(packet_dropped),
                45usize => Self(key_update),
                48usize => Self(key_space_discarded),
                53usize => Self(connection_started),
                54usize => Self(duplicate_packet),
                57usize => Self(transport_parameters_received),
                59usize => Self(datagram_sent),
                60usize => Self(datagram_sent__bytes__total),
                63usize => Self(datagram_received),
                64usize => Self(datagram_received__bytes__total),
                66usize => Self(datagram_dropped),
                67usize => Self(datagram_dropped__bytes__total),
                70usize => Self(handshake_remote_address_change_observed),
                71usize => Self(connection_id_updated),
                72usize => Self(ecn_state_changed),
                74usize => Self(connection_migration_denied),
                76usize => Self(handshake_status_updated),
                81usize => Self(tls_exporter_ready),
                82usize => Self(tls_handshake_failed),
                83usize => Self(path_challenge_updated),
                85usize => Self(tls_client_hello),
                87usize => Self(tls_server_hello),
                89usize => Self(rx_stream_progress),
                90usize => Self(rx_stream_progress__bytes__total),
                92usize => Self(tx_stream_progress),
                93usize => Self(tx_stream_progress__bytes__total),
                95usize => Self(keep_alive_timer_expired),
                96usize => Self(mtu_updated),
                100usize => Self(mtu_probing_complete_received),
                103usize => Self(slow_start_exited),
                107usize => Self(delivery_rate_sampled),
                108usize => Self(pacing_rate_updated),
                112usize => Self(bbr_state_changed),
                114usize => Self(dc_state_changed),
                120usize => Self(dc_path_created),
                121usize => Self(connection_closed),
                124usize => Self(version_information),
                125usize => Self(endpoint_packet_sent),
                126usize => Self(endpoint_packet_received),
                127usize => Self(endpoint_datagram_sent),
                131usize => Self(endpoint_datagram_received),
                134usize => Self(endpoint_datagram_dropped),
                138usize => Self(endpoint_connection_attempt_failed),
                140usize => Self(endpoint_connection_attempt_deduplicated),
                141usize => Self(platform_tx),
                142usize => Self(platform_tx__packets__total),
                144usize => Self(platform_tx__syscalls__total),
                146usize => Self(platform_tx__syscalls__blocked__total),
                148usize => Self(platform_tx__errors__total),
                150usize => Self(platform_tx__errors__dropped__total),
                152usize => Self(platform_tx_error),
                153usize => Self(platform_rx),
                154usize => Self(platform_rx__packets__total),
                156usize => Self(platform_rx__syscalls__total),
                158usize => Self(platform_rx__syscalls__blocked__total),
                160usize => Self(platform_rx__errors__total),
                162usize => Self(platform_rx__errors__dropped__total),
                164usize => Self(platform_rx_error),
                165usize => Self(platform_feature_configured),
                166usize => Self(platform_event_loop_wakeup),
                167usize => Self(platform_event_loop_sleep),
                169usize => Self(platform_event_loop_started),
    ...
    pub mod bool {
        ...
        impl Recorder {
            pub(crate) fn new(info: &'static Info) -> Self {
                match info.id {
                    25usize => Self(packet_lost__is_mtu_probe),
                    35usize => Self(recovery_metrics__congestion_limited),
                    99usize => Self(mtu_updated__search_complete),
                    _ => unreachable!("invalid info: {info:?}"),
                }
            }
        }
        ...
        impl Recorder {
            pub(crate) fn new(info: &'static Info, _variant: &'static info::Variant) -> Self {
                match info.id {
                    ...
                    8usize => Self(packet_sent__transmission_mode),
                    10usize => Self(packet_received__kind),
                    14usize => Self(frame_sent__packet),
                    15usize => Self(frame_sent__frame),
                    17usize => Self(frame_received__packet),
                    18usize => Self(frame_received__frame),
                    20usize => Self(connection_close_frame_received__packet),
                    22usize => Self(packet_lost__kind),
                    37usize => Self(congestion__source),
                    40usize => Self(ack_range_received__packet),
                    42usize => Self(ack_range_sent__packet),
                    44usize => Self(packet_dropped__reason),
                    46usize => Self(key_update__key_type),
                    47usize => Self(key_update__cipher_suite),
                    52usize => Self(key_space_discarded__space),
                    55usize => Self(duplicate_packet__kind),
                    56usize => Self(duplicate_packet__error),
                    69usize => Self(datagram_dropped__reason),
                    73usize => Self(ecn_state_changed__state),
                    75usize => Self(connection_migration_denied__reason),
                    80usize => Self(handshake_status_updated__status),
                    84usize => Self(path_challenge_updated__status),
                    98usize => Self(mtu_updated__cause),
                    101usize => Self(mtu_probing_complete_received__packet),
                    104usize => Self(slow_start_exited__cause),
                    113usize => Self(bbr_state_changed__state),
                    119usize => Self(dc_state_changed__state),
                    123usize => Self(connection_closed__error),
                    137usize => Self(endpoint_datagram_dropped__reason),
                    139usize => Self(endpoint_connection_attempt_failed__error),
                    _ => unreachable!("invalid info: {info:?}"),
                }
            }
        }
    ...
    impl Recorder {
        pub(crate) fn new(info: &'static Info) -> Self {
            match info.id {
                ...
                24usize => Self(packet_lost__bytes),
                27usize => Self(recovery_metrics__min_rtt),
                28usize => Self(recovery_metrics__smoothed_rtt),
                29usize => Self(recovery_metrics__latest_rtt),
                30usize => Self(recovery_metrics__rtt_variance),
                31usize => Self(recovery_metrics__max_ack_delay),
                32usize => Self(recovery_metrics__pto_count),
                33usize => Self(recovery_metrics__congestion_window),
                34usize => Self(recovery_metrics__bytes_in_flight),
                61usize => Self(datagram_sent__bytes),
                62usize => Self(datagram_sent__gso_offset),
                65usize => Self(datagram_received__bytes),
                68usize => Self(datagram_dropped__bytes),
                91usize => Self(rx_stream_progress__bytes),
                94usize => Self(tx_stream_progress__bytes),
                97usize => Self(mtu_updated__mtu),
                102usize => Self(mtu_probing_complete_received__mtu),
                106usize => Self(slow_start_exited__congestion_window),
                109usize => Self(pacing_rate_updated__bytes_per_second),
                110usize => Self(pacing_rate_updated__burst_size),
                111usize => Self(pacing_rate_updated__pacing_gain),
                128usize => Self(endpoint_datagram_sent__bytes),
                129usize => Self(endpoint_datagram_sent__bytes__total),
                130usize => Self(endpoint_datagram_sent__gso_offset),
                132usize => Self(endpoint_datagram_received__bytes),
                133usize => Self(endpoint_datagram_received__bytes__total),
                135usize => Self(endpoint_datagram_dropped__bytes),
                136usize => Self(endpoint_datagram_dropped__bytes__total),
                143usize => Self(platform_tx__packets),
                145usize => Self(platform_tx__syscalls),
                147usize => Self(platform_tx__syscalls__blocked),
                149usize => Self(platform_tx__errors),
                151usize => Self(platform_tx__errors__dropped),
                155usize => Self(platform_rx__packets),
                157usize => Self(platform_rx__syscalls),
                159usize => Self(platform_rx__syscalls__blocked),
                161usize => Self(platform_rx__errors),
                163usize => Self(platform_rx__errors__dropped),
                _ => unreachable!("invalid info: {info:?}"),
            }
        }
    }
...
mod timer {
    ...
    impl Recorder {
        pub(crate) fn new(info: &'static Info) -> Self {
            match info.id {
                49usize => Self(key_space_discarded__initial__latency),
                50usize => Self(key_space_discarded__handshake__latency),
                51usize => Self(key_space_discarded__one_rtt__latency),
                58usize => Self(transport_parameters_received__latency),
                77usize => Self(handshake_status_updated__complete__latency),
                78usize => Self(handshake_status_updated__confirmed__latency),
                79usize => Self(handshake_status_updated__handshake_done_acked__latency),
                86usize => Self(tls_client_hello__latency),
                88usize => Self(tls_server_hello__latency),
                115usize => Self(dc_state_changed__version_negotiated__latency),
                116usize => Self(dc_state_changed__no_version_negotiated__latency),
                117usize => Self(dc_state_changed__path_secrets__latency),
                118usize => Self(dc_state_changed__complete__latency),
                122usize => Self(connection_closed__latency),
                168usize => Self(platform_event_loop_sleep__processing_duration),
                _ => unreachable!("invalid info: {info:?}"),
            }
        }
    }
    ...
    pub mod nominal {
        ...
        impl Recorder {
            pub(crate) fn new(info: &'static Info, _variant: &'static info::Variant) -> Self {
                match info.id {
                    105usize => Self(slow_start_exited__latency),
                    _ => unreachable!("invalid info: {info:?}"),
                }
            }
        }
