...
fn convert_connection_error(e: s2n_quic::connection::Error) -> h3::quic::ConnectionErrorIncoming {
    match e {
        ...
        | error @ s2n_quic::connection::Error::MaxHandshakeDurationExceeded { .. }
        | error @ s2n_quic::connection::Error::ImmediateClose { .. }
        | error @ s2n_quic::connection::Error::EndpointClosing { .. }
        | error @ s2n_quic::connection::Error::InvalidConfiguration { .. }
        | error @ s2n_quic::connection::Error::Unspecified { .. }
        | error @ _ => ConnectionErrorIncoming::Undefined(Arc::new(error)),
    }
}
    ...
    B: Buf,
{
    ...
    fn poll_open_bidi(
        ...
        let stream = match ready!(self.conn.poll_open_bidirectional_stream(cx)) {
            Ok(x) => x,
            Err(e) => {
    ...
    fn poll_open_send(
        ...
        let stream = match ready!(self.conn.poll_open_send_stream(cx)) {
            Ok(x) => x,
            Err(e) => {
                return Poll::Ready(Err(StreamErrorIncoming::ConnectionErrorIncoming {
                    connection_error: convert_connection_error(e),
...
impl quic::RecvStream for RecvStream {
    ...
    fn poll_data(
        ...
        match ready!(self.stream.poll_receive(cx)) {
            Ok(Some(data)) => Poll::Ready(Ok(Some(data.into()))),
            Ok(None) => Poll::Ready(Ok(None)),
            Err(e) => Poll::Ready(Err(convert_read_error_to_stream_error(e))),
        }
    }
...
fn convert_read_error_to_stream_error(error: s2n_quic::stream::Error) -> StreamErrorIncoming {
    match error {
        ...
        error @ s2n_quic::stream::Error::InvalidStream { .. }
        | error @ s2n_quic::stream::Error::SendAfterFinish { .. }
        | error @ s2n_quic::stream::Error::MaxStreamDataSizeExceeded { .. }
        | error @ s2n_quic::stream::Error::NonReadable { .. }
        | error @ s2n_quic::stream::Error::NonWritable { .. }
        | error @ s2n_quic::stream::Error::SendingBlocked { .. }
        | error @ s2n_quic::stream::Error::NonEmptyOutput { .. }
        | error @ _ => StreamErrorIncoming::Unknown(Box::new(error)),
    }
}

fn convert_write_error_to_stream_error(error: s2n_quic::stream::Error) -> StreamErrorIncoming {
    match error {
        s2n_quic::stream::Error::StreamReset { error, .. } => {
            StreamErrorIncoming::StreamTerminated {
                error_code: error.into(),
    ...
    B: Buf,
{
    ...
    fn poll_ready(&mut self, cx: &mut task::Context<'_>) -> Poll<Result<(), StreamErrorIncoming>> {
        loop {
            ...
            if let Some(chunk) = self.chunk.as_mut() {
                if let Err(e) = ready!(self.stream.poll_send(chunk, cx)) {
                    return Poll::Ready(Err(convert_write_error_to_stream_error(e)));
                }
    ...
    fn poll_finish(&mut self, cx: &mut task::Context<'_>) -> Poll<Result<(), StreamErrorIncoming>> {
        ...
        if let Err(e) = self.stream.finish() {
            return Poll::Ready(Err(convert_write_error_to_stream_error(e)));
        }
        Ok(()).into()
    }
